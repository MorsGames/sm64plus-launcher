...
/// @func timer_get(timer_index)
/// @desc Returns either the duration the given timer, or the trigger function if the timer uses that.
/// @param {real} timer_index The index of the timer to get the remaining duration of.
...
function timer_get(timer_index) {
    for (var i = 0; i < ds_list_size(global.__timer_list); i++) {
        var _result = global.__timer_list[| i];
        if (_result.index == timer_index) {
...
/// @func timer_get_time(timer_index)
...
/// @param {real} timer_index The index of the timer to get the initial duration of.
/// @returns {real} How much time the given timer had initially, in frames or milliseconds.
function timer_get_time(timer_index) {
    for (var i = 0; i < ds_list_size(global.__timer_list); i++) {
        var _result = global.__timer_list[| i];
        if (_result.index == timer_index) {
...
/// @func timer_set_paused(timer_index, pause[OPTIONAL])
/// @desc Pauses the given timer, preventing it from progressing.
/// @param {real} timer_index The index of the timer to pause.
...
function timer_set_paused(timer_index) {
    for (var i = 0; i < ds_list_size(global.__timer_list); i++) {
        var _result = global.__timer_list[| i];
        if (_result.index == timer_index) {
            if (argument_count > 1)
...
/// @param {real} id The instance ID or the object index that the timers are assigned to. You can also use the "all" keyword.
...
/// @func timer_set_multiplier(timer_index, multiplier)
...
/// @param {real} timer_index The index of the timer to change the speed multiplier of.
...
function timer_set_multiplier(timer_index, multiplier) {
    for (var i = 0; i < ds_list_size(global.__timer_list); i++) {
        var _result = global.__timer_list[| i];
        if (_result.index == timer_index) {
...
/// @func timer_get_multiplier(timer_index)
...
/// @param {real} timer_index The index of the timer to get the speed multiplier of.
/// @returns {real} The speed multiplier of the given timer.
function timer_get_multiplier(timer_index) {
    for (var i = 0; i < ds_list_size(global.__timer_list); i++) {
        var _result = global.__timer_list[| i];
        if (_result.index == timer_index) {
...
/// @func timer_change(timer_index, duration, change_repeat_time[OPTIONAL])
/// @desc Changes the current remaining duration or the trigger of the given timer.
/// @param {real} timer_index The index of the timer to pause.
/// @param {real} duration The new remaining duration or the trigger function for the given timer.
...
function timer_change(timer_index, duration, change_repeat_time = false) {
    for (var i = 0; i < ds_list_size(global.__timer_list); i++) {
        var _result = global.__timer_list[| i];
        if (_result.index == timer_index) {
