	...
	if (round(_x_offset+room_width/2) < room_width && round(_x_offset+room_width/2) > -room_width) {
		...
		if (_total_options > 0) {
			...
			for (var i=scroll_offset; i<min(_total_options, scroll_offset+_options_per_page); i++) {
				...
				var _y_pos = (i-scroll_offset)*20 + add_shake(_hovered);
				...
				if (_current_category < 0) {
					...
					if (i == _total_options-1) {
						...
						if (selected)
							draw_option(room_width/2, 72 + _y_pos, keyboard_string + ((current_frames/32)%2 ? "_" : " "), 1.5);
						else
							draw_option(room_width/2, 72 + _y_pos, "✔[Add New Preset] ", 1.5);
					}
					else {
						
						var _name = filename_change_ext(filename_name(options[i]), "");
						var _name_lower = string_lower(_name);
						draw_option(room_width/2, 72 + _y_pos, (_name_lower == "recommended" || _name_lower == "vanilla" || _name_lower == "modern") ? _name : ("❓" + _name + " "), 1.5);
					}
				}
				else {
					...
					draw_option(room_width/2, 72 + _y_pos, _str, 1.5, _is_enabled);
				}
			}
